...
require_once "models/cliente.php";
    ...
    class usuarioController
    {
        ...
        {   
            ...
            if($_POST) //estou verificando se foi submetido a um formulario viA POST
            {
                ...
                if(empty($_POST["nome_usuario"])) 
                {
                    ...
                    // verificando se esta vazio, se sim passa uma mensagem 
                    //pra preencher o campo
                }
                else
                {
                ...
                // vericando se o email esta vazio, se tiver 
                //vazio vai aparecer pra preencher
                ...
                // se nao for vazio entra nesse else 
                 // se o campo nao passar na validacao
                 //o formato nao é de e-mail
                {
                    ...
                    {
                         // crio um novo objeto com email e senha fornecido no formularo
                        ...
                        $usuarioDAO = new usuarioDAO();
                         // verificando se o email ja existe, atraves do banco de dados
                        ...
                        {
                              // se retorno for array ou tiver mais de um elemento
                              // significa que o email ja existe
                            $erro = true;
                            $msg_erro[1] = "E-mail já existe,  por favor insira outro!";  // falando pro usuario que ja existe
                        }
                           // Se $retorno não é um array ou não possui elementos,
                         // significa que o email não existe e pode ser cadastrado
                        else
                        {
                            $msg_erro[1] = "";
                        }
                ...
                if(!$erro)
                {
                    ...
                    //print_r($cliente);
                   //criando um novo onjeto com os dados do formulario e incluindo cliente
                    ...
                    // gerando o hash, pra senha ficar mais segura
                    $usuario->password_hash();
                      // to instanciando o DAO para realizar o cadastro no banco de dados
                    $usuarioDAO = new usuarioDAO();
                    $retorno = $usuarioDAO->cadastrar($usuario);

                      // se o cadastro tiver correto, vai redireciona a pag de login
                    // pra ele poder entrar pag
                    if($retorno)
                    {
            ...
            require_once "views/cadastro.php"; // to chamando a views pra exibir o cadastro
        }
        ...
        public function exibir_login() // exibir_login é do login -> estou fazendo as function do login aqui, que pertecem ao objeto usuario
        {
            $msg_erro = array("","");
            if($_POST) //estou verificando se foi submetido a um formulario viA POST
            {
                ...
                if(empty($_POST["email"])) // VERIFICOU SE TA VAZIO, 
                                           //SIGNIFICA QUE NAO PRERENCHEU NADA
                {
                    $erro = true;
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        $usuario = new usuario(email:($_POST["email"]),senha: ($_POST["senha"])); // to referenciando o campo de email e senha com a variaveis de cada uma 
                        $usuarioDAO = new usuarioDAO(); // vou buscar no banco de dados
